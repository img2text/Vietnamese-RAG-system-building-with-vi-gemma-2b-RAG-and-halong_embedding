...
# Bắt đầu quá trình khởi tạo mô hình và tokenizer
print("Bắt đầu quá trình khởi tạo mô hình và tokenizer...")
tokenizer = AutoTokenizer.from_pretrained("himmeow/vi-gemma-2b-RAG")
model = AutoModelForCausalLM.from_pretrained(
    "himmeow/vi-gemma-2b-RAG",
    device_map="auto",
    torch_dtype=torch.bfloat16,  # Dùng precision thấp để tăng tốc nếu hỗ trợ
)
...
# Sử dụng f-string thay vì .format() để tối ưu thời gian xử lý
input_text = f"""
...
with torch.no_grad():  # Ngừng tính toán gradient để tiết kiệm bộ nhớ
    outputs = model.generate(
        **input_ids,
        ...
        temperature=0.7,  # Kiểm soát độ ngẫu nhiên
